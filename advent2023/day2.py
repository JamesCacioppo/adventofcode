...
class Game:
    ...
    def __init__(self, line_from_file: str):
        ...
        handfulls_string = line_from_file.split(":")[
            1
        ]  # A string that looks like "3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green"
        handfulls_list = handfulls_string.split(
            ";"
        )  # A list that looks like [' 3 blue, 4 red', ' 1 red, 2 green, 6 blue', ' 2 green']
        ...
        for handfull in self.handfull_list:
            ...
            if (
                handfull.num_green is not None
                and handfull.num_green > self.required_green
            ):
...
def sum_game_ids(
    list_of_games: List[Game], max_red: int, max_green: int, max_blue: int
) -> int:
    """
...
if __name__ == "__main__":
    ...
    print(
        f"The sum of valid game id's is {sum_game_ids(list_of_games, max_red=12, max_green=13, max_blue=14)}"
    )
